...
- 같은 모듈 안에서 동일한 파일명을 사용하지 않는다. DTO 파일명은 타입명과 달라도 된다.
- 고정된 date strategy 등 동일한 설정을 가진 `JSONEncoder`, `JSONDecoder`는 actor 또는 struct의 private 프로퍼티로 재사용한다.
...
- `Store 파일`의 상태 노출은 `docs/rules/architecture.md`의 MVI 규칙을 따른다.
...
### Error Handling
- 저장소 내부에서 새로 정의하는 `throw` 가능 API는 `docs/rules/architecture.md`의 typed throws 규칙을 따른다.
...
- 같은 모듈 안에 파일명이 중복되는 Swift 파일을 두지 않는다.
- 고정 설정 codec 객체를 호출마다 새로 생성하지 않는다.
...
- `Store`의 상태 노출 규칙은 `docs/rules/architecture.md`를 따르며, 이 문서에서 따로 재정의하지 않는다.
...
### Error Handling
- 저장소 내부에서 새로 정의하는 `throw` 가능 API의 typed throws 여부를 `docs/rules/architecture.md`와 다르게 적용하지 않는다.
...
- 외부 프로토콜, 시스템 API, 서드파티 API 시그니처가 untyped `throws`를 강제하는 경우에는 `docs/rules/architecture.md`의 예외와 같이 그 시그니처를 그대로 따른다.
...
- `Set`, `Dictionary`, `Map`처럼 자료구조 또는 키 기반 매핑 의미가 이름의 핵심인 경우에는 `List` 접미사를 강제하지 않는다.
- 외부 SDK, 시스템 API, 서버 응답 모델, 서드파티 타입 이름은 원래 시그니처와 계약을 우선 따른다.

## Checklist
- Apple / 로컬 모듈 / 외부 라이브러리 import 그룹 사이에 빈 줄이 있는가
- `View`·`Store` 헬퍼가 `도우미 extension`으로 내려갔는가
- `MARK 템플릿 적용 파일`에서 필요한 섹션이 3개 이상일 때 템플릿 이름·순서를 따랐는가
- 새 목록 이름에 `List` 접미사를 썼고 단순 복수형과 혼용하지 않았는가
- 모듈 밖 미사용 멤버에 `public`을 붙이지 않았는가
- 상속 없는 클래스를 `final class`로 선언했는가
- Store 상태·typed throws가 `docs/rules/architecture.md`와 맞는가
- `조기 종료 조건`을 `guard`로 정리했는가
