...
| FR-06 (정보 요청 버튼·툴팁·요청 페이지) | 포함 / 후속 이슈 | **버튼 노출은 포함** (사용자 피드백), 요청 페이지·툴팁은 후속 이슈 | Figma상 FR-05 두 화면 모두 우측 상단에 정보 요청 버튼 존재. 매칭실패 화면은 뒤로가기 없음, 직접찾기 화면만 뒤로가기 있음. 요청 페이지 디자인은 이 섹션에 없어 탭 액션 연결은 후속 |
