...
- 홈 관심 콘서트 섹션 타이틀 폰트를 `head1Semibold`로 변경한다.

## 작업 항목
- [ ] 홈 관심 콘서트 기본 정렬을 예매일로 변경한다.
...
- [ ] 홈 관심 콘서트 섹션 타이틀 폰트를 변경한다.
  - `HomeInterestConcertSectionView`의 `관심 콘서트` 타이틀 텍스트 폰트를 `head1Semibold`로 변경한다.
- [ ] 테스트와 수동 확인으로 QA 반영을 검증한다.
  ...
  - 관심 콘서트가 있는 홈에서 정렬 라벨, 카드 탭 이동, 진행중 문구, 타이틀 폰트를 수동 확인한다.
...
- 수동 확인
  ...
  - 홈 관심 콘서트 섹션 타이틀이 `head1Semibold` 스타일로 표시되는지 확인한다.
