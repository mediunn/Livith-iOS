...
- [ ] 새 관심 콘서트 설정 UI를 실제 화면 흐름에 연결한다.
  ...
  - 설정 완료 후 홈 갱신 또는 완료 화면 이동 정책을 기존 UX와 맞춰 연결한다.
  - 구현 후 계획 대비 구현 내용을 리뷰받고 `통과` 결과를 확인한 뒤 유저에게 알린다.
- [ ] 기존 단일 관심 콘서트 검색 흐름의 처리 방침을 정리한다.
  - `InterestConcertSearchView`와 `InterestConcertSearchStore`를 제거할지, 임시 보류할지 결정한다.
  - 제거하지 않는 경우 새 API 변경으로 컴파일이 깨지지 않도록 최소 수정한다.
  - 구현 후 계획 대비 구현 내용을 리뷰받고 `통과` 결과를 확인한 뒤 유저에게 알린다.
- [ ] 테스트를 갱신하고 추가한다.
...
| 설정 완료 UX | 완료 화면 이동 또는 토스트 노출 | 토스트 노출 | 유저 확인에 따라 설정 완료 화면은 제거하고 성공 토스트를 띄운다. |
| 기존 `InterestConcertSearchView` 처리 | 제거 또는 보류 | 구현 중 확인 | 새 UI로 진입점을 교체하되 미사용 코드 제거 여부는 컴파일 영향과 사용처 확인 후 최소 변경으로 결정한다. |
