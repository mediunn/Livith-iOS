...
**상황**
- TestFlight에 올라간 빌드 번호가 `20260727.355`(03:55) 등으로, 실제 배포한 낮 시각(KST)과 9시간 차이.

**문제**
- `Time.now.strftime`이 GitHub-hosted 러너의 로컬 시각(UTC)을 찍어 KST 배포 시각과 불일치. (로컬 맥은 KST라 테스트 때는 정상이었음.)
- 부수 확인: Apple은 CFBundleVersion 숫자 그룹의 앞자리 0을 제거(`0355`→`355`). 정수 비교라 정렬은 유지되어 기능 문제는 없음.

**원인**
- 러너 타임존이 UTC. Fastfile이 타임존을 명시하지 않아 러너 TZ에 의존.

**해결**
- `Time.now.getlocal("+09:00")`로 KST 고정. 러너 TZ와 무관하게 배포 시각이 KST로 찍힘.

**교훈**
- CI 러너는 UTC 기본. "배포 시각"처럼 사람이 읽는 시간은 타임존을 코드에 명시한다.
